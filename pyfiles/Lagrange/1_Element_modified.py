...
alpha = 0 #np.array([-0.57735,0.57735,0.57735,-0.57735])     # Gauss points 
beta  = 0 #np.array([-0.57735,-0.57735,0.57735,0.57735])
W_Cs  = 4                                                 #weight for gauss quadrature in the cross section
    ...
    for j in range(n):
        ...
        for tau_en,tau in enumerate(range(L_poly)):
            for s_en,s in enumerate(range(L_poly)):
                ...
                K_xx =  np.asscalar(C_22*(W_Cs*F_tau_x*F_s_x*J_Cs*W_Length*np.sum(Shape_func[i]*Shape_func[j]*J_Length)) + C_66*(W_Cs*F_tau_z*F_s_z*J_Cs*W_Length*np.sum(Shape_func[i]*Shape_func[j]*J_Length)) + C_44*(W_Cs*Lag_poly[tau]*Lag_poly[s]*J_Cs*W_Length*np.sum(N_Der[i]*N_Der[j]*J_Length)))
                K_xy =  np.asscalar(C_23*(W_Cs*Lag_poly[tau]*F_s_x*J_Cs*W_Length*np.sum(N_Der[i]*Shape_func[j]*J_Length)) + C_44*(W_Cs*F_tau_x*Lag_poly[s]*J_Cs*W_Length*np.sum(Shape_func[i]*N_Der[j]*J_Length)))
                K_xz =  np.asscalar(C_12*(W_Cs*F_tau_z*F_s_x*J_Cs*W_Length*np.sum(Shape_func[i]*Shape_func[j]*J_Length)) + C_66*(W_Cs*F_tau_x*F_s_z*J_Cs*W_Length*np.sum(Shape_func[i]*Shape_func[j]*J_Length)))     
                K_yx =  np.asscalar(C_44*(W_Cs*Lag_poly[tau]*F_s_x*J_Cs*W_Length*np.sum(N_Der[i]*Shape_func[j]*J_Length)) + C_23*(W_Cs*F_tau_x*Lag_poly[s]*J_Cs*W_Length*np.sum(Shape_func[i]*N_Der[j]*J_Length)))
                K_yy =  np.asscalar(C_55*(W_Cs*F_tau_z*F_s_z*J_Cs*W_Length*np.sum(Shape_func[i]*Shape_func[j]*J_Length)) + C_44*(W_Cs*F_tau_x*F_s_x*J_Cs*W_Length*np.sum(Shape_func[i]*Shape_func[j]*J_Length)) + C_33*(W_Cs*Lag_poly[tau]*Lag_poly[s]*J_Cs*W_Length*np.sum(N_Der[i]*N_Der[j]*J_Length)))
                K_yz =  np.asscalar(C_55*(W_Cs*Lag_poly[tau]*F_s_z*J_Cs*W_Length*np.sum(N_Der[i]*Shape_func[j]*J_Length)) + C_13*(W_Cs*F_tau_z*Lag_poly[s]*J_Cs*W_Length*np.sum(Shape_func[i]*N_Der[j]*J_Length)))
                K_zx =  np.asscalar(C_12*(W_Cs*F_tau_x*F_s_z*J_Cs*W_Length*np.sum(Shape_func[i]*Shape_func[j]*J_Length)) + C_66*(W_Cs*F_tau_z*F_s_x*J_Cs*W_Length*np.sum(Shape_func[i]*Shape_func[j]*J_Length))) 
                K_zy =  np.asscalar(C_13*(W_Cs*Lag_poly[tau]*F_s_z*J_Cs*W_Length*np.sum(N_Der[i]*Shape_func[j]*J_Length)) + C_55*(W_Cs*F_tau_z*Lag_poly[s]*J_Cs*W_Length*np.sum(Shape_func[i]*N_Der[j]*J_Length))) 
                K_zz =  np.asscalar(C_11*(W_Cs*F_tau_z*F_s_z*J_Cs*W_Length*np.sum(Shape_func[i]*Shape_func[j]*J_Length)) + C_66*(W_Cs*F_tau_x*F_s_x*J_Cs*W_Length*np.sum(Shape_func[i]*Shape_func[j]*J_Length)) + C_55*(W_Cs*Lag_poly[tau]*Lag_poly[s]*J_Cs*W_Length*np.sum(N_Der[i]*N_Der[j]*J_Length)))
